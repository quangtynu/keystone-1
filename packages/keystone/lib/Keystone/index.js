...
module.exports = class Keystone {
  constructor({
    ...
    this._providers = [
      ...
      new VersionProvider({ appVersion, schemaNames: this._schemaNames }),
    ];
  ...
  createList(key, config, { isAuxList = false } = {}) {
    ...
    this._listCRUDProvider.listsArray.push(list);
    list.initFields();
    return list;
  }
  ...
  getTypeDefs({ schemaName }) {
    ...
    return [
      ...
      queries.length && `type Query { ${queries.join('\n')} }`,
  ...
  async prepare({
    ...
    const middlewares = flattenDeep([
      // appVersionMiddleware(this.appVersion),
